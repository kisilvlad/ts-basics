...
function getMostPopularCourseType(): CourseType {
  const count = schedule.reduce<Record<CourseType, number>>(
    (acc, l) => {
      const course = courses.find((c) => c.id === l.courseId);
      if (course) acc[course.type]++;
      return acc;
    },
...
function reassignClassroom(
  ...
  if (conflict) {
    console.log(
      `(Error) Cannot reassign: ${conflict.type} with lesson ID ${conflict.lessonDetails.id}`
    );
    return false;
  }
  ...
  console.log(
    `(DONE) Reassigned lesson ${lessonId} to classroom ${newClassroomNumber}`
  );
  return true;
}
...
function test(): void {
  ...
  console.log(
    "Available classrooms Monday 8:30-10:00:",
    findAvailableClassrooms("8:30-10:00", "Monday")
  );
  console.log("Professor 1 schedule:", getProfessorSchedule(1));
  console.log(
    "Classroom A101 utilization:",
    getClassroomUtilization("A101").toFixed(2) + "%"
  );
